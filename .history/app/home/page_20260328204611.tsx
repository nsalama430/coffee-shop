...
const mapProductsToFoodItems = (products: any[]): any[] => {
  return products.map((product) => {
    ...
    if (foodItemSizes.length === 0) {
      ...
      const isDerivative = product.categoryId === 'مشتقات القهوة' || product.category === 'مشتقات القهوة';
      const derivativeType = product.derivativeType || "نسكافيه كلاسيك";
      const flavor = product.flavor || "كلاسيك";
      const displayType = isDerivative 
        ? (derivativeType === 'كابيتشينو' ? `كابيتشينو - ${flavor}` : derivativeType)
        : "";
...
const menuStructure = [
  ...
  },
  {
    id: 'اسـبريـسو',
    title: 'إسبريسو',
    rows: ['100', '90:10', '80:20', '70:30', '60:40', '50:50', '40:60', '30:70', '20:80', '10:90']
  },
  {
    id: 'سبشيال',
    title: 'السبشيال',
  ...
  },
  {
    id: 'مشتقات القهوة',
    title: 'مشتقات القهوة',
    rows: ['نسكافيه كلاسيك', 'نسكافيه جولد', 'بندق فلافر', 'بندق قطع', 'هوت شوكلت', 'قهوة شوكولاتة', 'كابيتشينو - كلاسيك', 'كابيتشينو - بندق', 'كابيتشينو - كراميل', 'كابيتشينو - فانيليا']
  }
];
  ...
  return (
    <div className="pb-8" dir="rtl">
        ...
        isBannersLoading ? (
          ...
          <section className="w-full mb-8" dir="rtl">
            <Carousel 
              ...
              plugins={visibleBanners.length > 1 ? [autoplayPlugin.current] : []}
            >
              <CarouselContent>
      ...
      {slug.length > 0 && <h1 className="text-3xl font-bold mb-8 text-primary">
          {`${categoryName.replace('ـ', '')}${blendType ? ` - ${blendType}` : ''}${roastLevel ? ` ${roastLevel.replace(' (محروق)', '')}` : ''}`}
        </h1>}
              ...
              {category.groups ? (
                  ...
                  {category.groups.map(group => {
                    ...
                    return (
                      <div key={group.title} className="bg-gray-50/50 dark:bg-gray-900/20 rounded-3xl p-4 md:p-8 border border-gray-100 dark:border-gray-800">
                        ...
                        const viewAllLink = `/home/${urlFriendly(category.id)}/${urlFriendly(group.blendType)}/${urlFriendly(roastLevel)}`;
                ...
                <div className="grid grid-cols-1 gap-12 px-2 lg:px-6">
                  <div className="bg-gray-50/50 dark:bg-gray-900/20 rounded-3xl p-4 md:p-8 border border-gray-100 dark:border-gray-800">
                    <div className="space-y-2">
                  {category.rows.map(rowValue => {
                    const rowItems = categoryItems.filter(item => 
                    category.id === 'اسـبريـسو' ? item.ratio === rowValue : 
                    category.id === 'مشتقات القهوة' ? item.displayType === rowValue : item.roastLevel === rowValue
                    );
                        if (rowItems.length === 0) return null;

                    const displayTitle = rowValue === 'غامق (محروق)' ? 'غامق' : rowValue;

                    return <ProductRow key={rowValue} title={displayTitle} items={rowItems} viewAllLink={`/home/${urlFriendly(category.id)}`} />;
                      })}
                    </div>
                  </div>
                </div>
                ...
                <div className="px-2 lg:px-6">
                  <div className="bg-gray-50/50 dark:bg-gray-900/20 rounded-3xl p-4 md:p-8 border border-gray-100 dark:border-gray-800">
                <ProductRow title="منتجات متنوعة" items={categoryItems} viewAllLink={`/home/${urlFriendly(category.id)}`} />
                  </div>
                </div>
              )}
            </section>
