                ...
                <div className="grid grid-cols-1 gap-12 px-2 lg:px-6">
                  ...
                  {category.rows.map(rowValue => {
                    const rowItems = categoryItems.filter(item => 
                      category.id === 'اسـبريـسو' ? item.ratio === rowValue : item.roastLevel === rowValue
                    );
                        if (rowItems.length === 0) return null;

                    const displayTitle = rowValue === 'غامق (محروق)' ? 'غامق' : (category.id === 'اسـبريـسو' ? `نسبة ${rowValue}` : rowValue);
